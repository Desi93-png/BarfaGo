...
<body>
  ...
  <nav class="navbar">
    ...
    <div class="navbar-nav">
      ...
      <a href="pendapatan.php" style="color: #08C2FF;">Lap.Pendapatan</a>
      ...
      <div class="breadcrumb">
        <div class="card-body">
          ...
          <table class="table" id="tables">
              ...
              <?php
                $no = 1;
                $grand_totalpendapatan = 0;
                $query = mysqli_query($connect, "
                    SELECT 
                        DATE(p.tanggal_pesan) AS tanggal,
                        SUM(dp.jumlah) AS total_produk_terjual,
                        SUM(CASE WHEN dp.nama_produk = 'Le Minerale' THEN dp.jumlah ELSE 0 END) AS le_minerale_terjual,
                        SUM(CASE WHEN dp.nama_produk = 'Aqua' THEN dp.jumlah ELSE 0 END) AS aqua_terjual,
                        SUM(CASE WHEN dp.nama_produk = 'Air Isi Ulang' THEN dp.jumlah ELSE 0 END) AS isi_ulang_terjual,
                        SUM(CASE WHEN dp.nama_produk = 'Galon Kosong' THEN dp.jumlah ELSE 0 END) AS galon_kosong_terjual,
                ...
                while ($row = mysqli_fetch_assoc($query)) {
                    $grand_totalpendapatan += $row['total_pendapatan'];
                ?>
                  <tr style="text-align:left;">
                      <td><?php echo $no++; ?>.</td>
                      <td><?php echo $row['tanggal']; ?></td>
                      <td><?php echo $row['jumlah_pesanan']; ?></td>
                      <td><?php echo $row['total_produk_terjual']; ?></td>
                      <td><?php echo $row['le_minerale_terjual']; ?></td>
                      <td><?php echo $row['aqua_terjual']; ?></td>
                      <td><?php echo $row['isi_ulang_terjual']; ?></td>
                      <td><?php echo $row['galon_kosong_terjual']; ?>
          ...
          <?php
            $query_terlaris = mysqli_query($connect, "
            SELECT 
                nama_produk,
            ...
            LIMIT 1
            "); 
            $produk_terlaris = mysqli_fetch_assoc($query_terlaris);
          ?>

        <div class="desc">
          <div class="info-row">
            <span class="label">Produk Terlaris</span>
            <span class="value">: <?php echo $produk_terlaris['nama_produk']; ?> (<?php echo $produk_terlaris['total_terjual']; ?> terjual)</span>
          </div>
          <div class="info-row">
            <span class="label">Total Pendapatan</span>
            <span class="value">: Rp<?php echo number_format($grand_totalpendapatan, 0, ',', '.'); ?></span>
          </div>
        </div>
